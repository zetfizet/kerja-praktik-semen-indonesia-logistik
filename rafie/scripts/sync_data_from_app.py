...
# Mapping tabel yang akan di-sync
...
TABLE_MAPPINGS = {
    ...
    "perangkat_gps_driver": "perangkat_gps_driver",
    "rekening_driver": "rekening_driver"
}
...
def get_table_structure(conn, table_name):
    """Get struktur tabel (kolom dan tipe data)"""
    cursor = conn.cursor()
    try:
        cursor.execute(f"""
...
def copy_data(source_conn, target_conn, source_table, target_table):
    ...
    try:
        ...
        if not rows:
            print(f"⚠️  {source_table} is empty")
            return 0
