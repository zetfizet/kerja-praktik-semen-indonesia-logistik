...
script_paths = [
    ...
    '/home/xfrih/project/airflow/scripts',  # Local path
    '/home/xfrih/project/airflow/dags',  # Local dags path
]
...
def fetch_weather_data():
    ...
    # Use service name 'postgres' when running in container
    # Use internal port 5432 (not 5433 which is host port)
    DB_CONFIG = {
        'host': 'postgres',
        'port': 5432,
        'database': 'warehouse',
    ...
    def parse_weather_data(api_data, location_info):
        ...
        try:
            ...
            latitude = _safe_float(
                lokasi_info.get('lat')
                or lokasi_info.get('latitude')
                or location_info.get('latitude')
                or location_info.get('lat')
            )
            longitude = _safe_float(
                lokasi_info.get('lon')
                or lokasi_info.get('longitude')
                or location_info.get('longitude')
                or location_info.get('lon')
            )
    ...
    def create_weather_table_if_not_exists():
        ...
        try:
            ...
            cursor.execute("""
                ALTER TABLE public.fact_weather_hourly
...
def verify_weather_data():
    ...
    try:
        ...
        print(f"\n  Latest 5 records:")
        print(f"  {'Lokasi':<15} {'Lat':<10} {'Lon':<10} {'Waktu':<17} {'Cuaca':<10} {'Suhu':<5} {'Created At':<20} {'Last Updated':<20} {'Age':<6} {'Status':<8}")
        print(f"  {'-'*15} {'-'*10} {'-'*10} {'-'*17} {'-'*10} {'-'*5} {'-'*20} {'-'*20} {'-'*6} {'-'*8}")
        for row in latest:
            lokasi, lat, lon, waktu, cuaca, suhu, created, updated, age, status = row
            lat_text = lat if lat is not None else 'N/A'
            lon_text = lon if lon is not None else 'N/A'
            print(f"  {lokasi:<15} {str(lat_text):<10} {str(lon_text):<10} {waktu:<17} {cuaca:<10} {suhu:>3}°C {created:<20} {updated:<20} {age if age else 'N/A':>5} {status or 'N/A':<8}")
        print("=" * 80)
