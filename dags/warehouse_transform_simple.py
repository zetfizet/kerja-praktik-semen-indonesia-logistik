...
def transform_route_summary():
    """
    Build analytics.route_summary from fact_route_metrics.
    """
    conn = psycopg2.connect(**TARGET_DB_CONFIG)
    cursor = conn.cursor()

    try:
        print("🔄 Transforming route summary...")
        ...
        cursor.execute(
            ...
            SELECT COUNT(*)
            FROM information_schema.tables
            WHERE table_schema = 'public' AND table_name = 'fact_route_metrics'
            """
        )

        if cursor.fetchone()[0] == 0:
            print("  ⚠ fact_route_metrics table not found, skipping")
            return

        cursor.execute("DROP TABLE IF EXISTS analytics.route_summary")
        cursor.execute(
            """
            CREATE TABLE analytics.route_summary AS
            SELECT
                origin_name,
                destination_name,
            ...
            FROM public.fact_route_metrics
        ...
        cursor.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_route_summary_origin_destination
                ON analytics.route_summary(origin_name, destination_name)
            """
        )

        conn.commit()

        cursor.execute("SELECT COUNT(*) FROM analytics.route_summary")
        count = cursor.fetchone()[0]
        print(f"✓ route_summary: {count} route pairs")
...
def transform_route_weather_summary():
    """
    Build analytics.route_weather_summary combining routing and weather context.
    """
    conn = psycopg2.connect(**TARGET_DB_CONFIG)
    cursor = conn.cursor()

    try:
        print("🔄 Transforming route weather summary...")
        ...
        has_routes = cursor.fetchone()[0] > 0
        ...
        has_weather = cursor.fetchone()[0] > 0

        if not has_routes or not has_weather:
            print("  ⚠ route/weather source table missing, skipping")
            return

        cursor.execute("DROP TABLE IF EXISTS analytics.route_weather_summary")
        cursor.execute(
            """
            CREATE TABLE analytics.route_weather_summary AS
            SELECT
                r.origin_name,
                r.destination_name,
                AVG(r.distance_km) AS avg_distance,
                AVG(r.duration_min) AS avg_duration,
                w.cuaca,
                AVG(w.suhu_celsius) AS avg_temp
            FROM public.fact_route_metrics r
            JOIN public.fact_weather_hourly w
              ON r.origin_name ILIKE '%' || w.lokasi || '%'
              OR r.destination_name ILIKE '%' || w.lokasi || '%'
                        WHERE r.distance_km < 100
            GROUP BY r.origin_name, r.destination_name, w.cuaca
            """
        )

        cursor.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_route_weather_summary_route
                ON analytics.route_weather_summary(origin_name, destination_name)
            """
        )

        conn.commit()

        cursor.execute("SELECT COUNT(*) FROM analytics.route_weather_summary")
        count = cursor.fetchone()[0]
        print(f"✓ route_weather_summary: {count} rows")
...
task_route_summary = PythonOperator(
    task_id='transform_route_summary',
    python_callable=transform_route_summary,
    dag=dag,
)

task_route_weather_summary = PythonOperator(
    task_id='transform_route_weather_summary',
    python_callable=transform_route_weather_summary,
    dag=dag,
)
...
[task_table_counts, task_orders, task_customers, task_delivery, task_inventory, task_route_summary, task_route_weather_summary] >> task_summary
